...
window.addEventListener('keydown', e => {
    if(e.key === 'Backspace') {
        window.location.href = '/dashboard/reportes/compras-por-tiempo';
    }
});
...
const generateReport = () => {
    ...
    fetch(`http://localhost:5000/dashboard/reportes/compras-por-tiempo/${fechaEmpiezo.value}/${fechaFinal.value}`, {
        method: 'GET',
        mode: 'cors', 
        cache: 'no-cache',
    ...
    .then(compras => {
        ...
        returnAnchor.href = '/dashboard/reportes/compras-por-tiempo';
...
const downloadReport = async() => {
    ...
    .then(result => {
        if(!result.canceled) {
            fetch(`http://localhost:5000/dashboard/reportes/compras-por-tiempo-download`, {
                method: 'POST',
                mode: 'cors', 
                cache: 'no-cache',
                credentials: 'same-origin',
